...
class EPHprocessor: 
    ...
    #                      label_list: list[str],
    #                      expression: str,
    #                      frame_idx_list: list[list[int]], # at the same time, different folder may have different frame indices to analyze
    ...
    #             density_output = np.full((max_density_len, len(frame_idx_list)), np.nan)
    #             coupling_output = np.full((max_coupling_len, len(frame_idx_list)), np.nan)
    #             ek_output = np.full((max_ek_len, len(frame_idx_list)), np.nan)
    ...
    #             density_header = "  ".join([f"f_{frame_idx}" for frame_idx in frame_idx_list])
    #             coupling_header = "  ".join([f"f_{frame_idx}" for frame_idx in frame_idx_list])
    #             # ek_header = "  ".join([f"f_{frame_idx}" for frame_idx in frame_idx_list])
    ...
    #             c_vals = coupling_output[:, i]
    #             c_vals = c_vals[~np.isnan(c_vals)]

    #             d_vals = density_output[:, i]
    #             d_vals = d_vals[~np.isnan(d_vals)]

    #             ek_vals = ek_output[:, i]
    #             ek_vals = ek_vals[~np.isnan(ek_vals)]
    #             # convert to velocity unit of Å/ps
    #             vel_vals = np.sqrt(2 * ek_vals * 1.60218e-19 / 72.64 * 1e20) # (eV to J) / (mass in kg) * (m² to Å²) → velocity in Å/ps
    #             ax.scatter(ek_vals, d_vals, label=f"{label_list[j]} {time_list[frame_idx_id]:.0f} fs", s=1, alpha=0.5)

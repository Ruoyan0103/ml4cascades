...
class CascadeCalculator(TurboGAPCalculator): 
    ...
    def thermalize_electronic(self, equ_md_steps: int, temp: float, 
        ...
        if atomsfile is None:
            thermalized_atoms = read(os.path.join(thermalize_dir, 'thermalize_atomic', 'trajectory_out.xyz'), format='extxyz', index=-1)
            thermalized_atoms_file = os.path.join(thermalize_dir, 'thermalize_atomic', 'thermalized_atoms.xyz')
